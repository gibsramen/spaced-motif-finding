...
def gappedMotifGibbsAlignment(Dna, G, k1, k2, n):
	...
	for d in Dna:
		...
		if len(G) > 1:
			k2_start_ind = choice(range(k1_start_ind + g_min, k1_start_ind + max(G)))
		else:
			k2_start_ind = k1_start_ind + G[0]
		motifs.append(d[k1_start_ind:k1_start_ind + k1] + d[k2_start_ind : k2_start_ind + k2])
...
def pipeline_gibbs_sampler_with_alignment(Dna, G, k1, k2, num_iters, num_runs):
	...
	for _ in range(num_runs):
		run_results.append(gappedMotifGibbsAlignment(Dna, G, k1, k2, num_iters))
...
if __name__ == '__main__':
	...
	parser.add_argument('--num_iters', metavar = 'num_iters', type=int, help='Number of Gibs sampling iterations.',required=True)
	parser.add_argument('--num_runs', metavar = 'num_runs', type=int, help='Number of random restarts for Gibbs sampling.',required=True)
	...
	with open(logFN,'w') as f:
		...
		f.write('Sampling runs used to define consensus: ' + str(args.num_runs) + '\n')
